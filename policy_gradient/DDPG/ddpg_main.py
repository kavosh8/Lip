...
run=sys.argv[1]
k=sys.argv[2]
env_test_name=sys.argv[3]
env_name='Pendulumlearned'+str(k)+'-v0'





flags.DEFINE_string('env_name', env_name, 'environment name in gym.')
flags.DEFINE_boolean('env_render', False, 'whether render environment (display).')
flags.DEFINE_boolean('env_monitor', False, 'whether use gym monitor.')
...
if env_name=='Pendulum-v0' or env_test_name=='Pendulum-v0':
    MAX_EPISODES = 250
...
def main(_):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        if FLAGS.env_name=='Pendulum-v0' or env_test_name=='Pendulum-v0':  
        ...
        agent = DDPGAgent(sess, action_type, actor, critic, GAMMA, env,env_test_name, replay_buffer, noise=noise, exploration_episodes=EXPLORATION_EPISODES,\
        ...
        agent.train(run,env_name,env_test_name)
        env.close()
import t
